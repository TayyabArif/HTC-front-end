...
export const WeatherPlayer = (props) => {
  ...
  const onSliderChange = (event, value) => {
    ...
    case 'radar':
      ...
      case 100:
        props.setQueryTime({
          conus: availableTimes.current[availableTimes.current.length - 1],
          alaska: availableTimesAlaska.current[availableTimesAlaska.current.length - 1]
        })
      }
      break
    case 'weather':
  ...
  return (
    <Box hidden={props.hidden}>
      <Fade
        className={actualWidth <= mobileBreakpoint ? classes.playerContainerMinMobile : classes.playerContainerMin}
        timeout={2000}
        in={playerHidden}
                ...
                <Grid container spacing={2} justifyContent={'space-between'}>
                  ...
                  <Grid item>
                    <IconButton onClick={onHidePlayer} className={classes.playerHideContainer}>
                      {actualWidth <= mobileBreakpoint
                        ? <KeyboardArrowLeftOutlined className={classes.playerHide} />
      ...
      <Slide
        className={actualWidth <= mobileBreakpoint ? classes.playerContainerMobile : classes.playerContainer}
        direction={actualWidth <= mobileBreakpoint ? 'left' : 'up'}
        timeout={1000} in={!playerHidden}
        container={containerRef.current}>
        <Paper>
          <Box p={1}>
            <Box pl={1}>
              <Grid container spacing={2} justifyContent={'space-between'}>
                ...
                <Grid item>
                  <IconButton onClick={onHidePlayer} className={classes.playerHideContainer}>
                    {actualWidth <= mobileBreakpoint
                      ? <KeyboardArrowRightOutlined className={classes.playerHide} />
            ...
            <Box pl={1}>
              <Grid container spacing={2}>
                ...
                <Grid item>
                  <SelectRadiosNoBorder
                    handleChange={handleAFChange}
                    dataDefault={10}
                    dataElement={'AF'}
                    dataItems={AFStateOptions}
                    selectedOption={AFState}

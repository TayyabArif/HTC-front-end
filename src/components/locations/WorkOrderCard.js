      ...
      <Box className={'sitesCard'} pb={0.5} style={props.style} >
        ...
        >
          ...
          <Box display="flex" width="100%">
            <Box flex={5}>
              {(info?.customer_po && info?.customer_po !== '') && <Typography className={classes.woNumber} display="inline" align='left'>
                WO# {info?.customer_po}
              </Typography>}
              {(info?.client_tracking_number && info?.client_tracking_number) && <Typography marginBottom={0.3} className={classes.clientTracking} align='left'>
                {t('locations.work_orders.tracking')}# {info?.client_tracking_number}
              </Typography>}
              {(info?.call_type && info?.call_type !== '') && <Typography marginBottom={0.3} className={classes.woType} align='left'>
                {info?.call_type}
              </Typography>}
              {(info?.open_date && info?.open_date !== '') && <Typography marginBottom={0.3} className={classes.startLabel} align='left'>
                {t('locations.work_orders.start')} {info?.open_date ? moment.unix(info.open_date).format('DD/MM/YY hh:mm a') : ''}
              </Typography>}
              {getServicesLabel() !== '' && <Typography marginBottom={0.3} className={classes.woType} align='left'>
